...
auto target_condition = [](const vec& ss) { return (ss[0] >= 5.0 && ss[0] <= 7.0) && (ss[1] >= 5.0 && ss[1] <= 7.0); };
auto avoid_condition = [](const vec& ss) { return (ss[0] >= -2.0 && ss[0] <= 2.0 ) && (ss[1] >= -2.0 && ss[1] <= 2.0); };

//dynamics - 1 parameters
...
int main() {
    ...
    mdp.setNoise(NoiseType::CUSTOM, false);
    mdp.setStdDev(sigma);
