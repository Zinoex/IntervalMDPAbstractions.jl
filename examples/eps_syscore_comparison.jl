...
car_parking = EpsSySCoReComparisonProblem(
    ...
    (prop, time_horizon) -> FiniteTimeReachAvoid(
        IntervalMDP.reach(prop),
        IntervalMDP.avoid(prop),
        time_horizon,
    ),
...
function benchmark_intervalsyscore(mdp, lower_spec, upper_spec, time_horizons)
    ...
    @assert time_horizon(system_property(lower_spec)) ==
            time_horizon(system_property(upper_spec))
...
function construct_model(problem, state_split, time_horizon = 1)
    ...
    mdp, lower_spec, upper_spec =
        problem.constructor(state_split, problem.input_split, time_horizon)
...
function benchmark_system(problem, state_split, time_horizon)
    ...
    mean_V_eps, max_V_eps =
        benchmark_intervalsyscore(mdp, lower_spec, upper_spec, [time_horizon])
...
function benchmark_system(problem::EpsSySCoReComparisonProblem, state_split)
    ...
    println((
        ...
        state_split,
        "memory (MB)",
        Base.summarysize(mdp) / 1024^2,
    ))

    mean_V_eps, max_V_eps =
        benchmark_intervalsyscore(mdp, lower_spec, upper_spec, problem.time_horizons)

    for (time_horizon, mean_V_ep, max_V_ep) in
        zip(problem.time_horizons, mean_V_eps, max_V_eps)
        println((
            "time_horizon",
            time_horizon,
            "mean_V_eps",
