...
car_parking = IntervalSySCoReComparisonProblem(
    "car_parking",
    (state_split, input_split, time_horizon) -> car_parking_direct(
        time_horizon;
        range_vs_grid = :range,
        sparse = true,
    ...
    (state_split, input_split, time_horizon) -> car_parking_decoupled(
        time_horizon;
        range_vs_grid = :range,
        state_split = state_split,
        input_split = input_split,
...
robot_2d_reachability = IntervalSySCoReComparisonProblem(
    ...
    (state_split, input_split, time_horizon) -> robot_2d_direct(
        time_horizon;
        spec = :reachability,
        sparse = true,
        state_split = state_split,
    ...
    (state_split, input_split, time_horizon) -> robot_2d_decoupled(
        time_horizon;
        spec = :reachability,
        state_split = state_split,
        input_split = input_split,
...
bas4d = IntervalSySCoReComparisonProblem(
    "bas4d",
    (state_split, input_split, time_horizon) -> building_automation_system_4d_direct(
        time_horizon;
        sparse = true,
        state_split = state_split,
        input_split = input_split,
    ),
    (state_split, input_split, time_horizon) ->
        building_automation_system_4d_decoupled(
            time_horizon;
            state_split = state_split,
            input_split = input_split,
        ),
...
van_der_pol = IntervalSySCoReComparisonProblem(
    "van_der_pol",
    (state_split, input_split, time_horizon) -> van_der_pol_direct(
        time_horizon;
        state_split = state_split,
        input_split = input_split,
    ),
    (state_split, input_split, time_horizon) -> van_der_pol_decoupled(
        time_horizon;
        state_split = state_split,
        input_split = input_split,
    ),
...
nndm_cartpole = IntervalSySCoReComparisonProblem(
    "nndm_cartpole",
    (state_split, input_split, time_horizon) ->
        action_cartpole_direct(time_horizon; sparse = true),
    (state_split, input_split, time_horizon) ->
        action_cartpole_decoupled(time_horizon; sparse = false),
    (20, 20, 24, 20),
    (1,),
    10,
)
...
linear6d = IntervalSySCoReComparisonProblem(
    "linear6d",
    (state_split, input_split, time_horizon) ->
        almost_identity_direct(6, time_horizon; sparse = true, state_split_per_dim = 8),
    (state_split, input_split, time_horizon) -> almost_identity_decoupled(
        6,
        time_horizon;
        sparse = true,
...
linear7d = IntervalSySCoReComparisonProblem(
    "linear7d",
    (state_split, input_split, time_horizon) ->
        almost_identity_direct(7, time_horizon; sparse = true, state_split_per_dim = 8),
    (state_split, input_split, time_horizon) -> almost_identity_decoupled(
        7,
        time_horizon;
        sparse = true,
...
dubins_car_gp_dkl = IntervalSySCoReComparisonProblem(
    "dubins_car_gp_dkl",
    (state_split, input_split, time_horizon) ->
        dubins_car_direct(time_horizon; sparse = true),
    (state_split, input_split, time_horizon) ->
        dubins_car_decoupled(time_horizon; sparse = false),
    (80, 16, 20),
    (7,),
    10,
)
...
linear_stochastically_switched = IntervalSySCoReComparisonProblem(
    ...
    (state_split, input_split, time_horizon) ->
        linear_stochastically_switched_direct(time_horizon; state_split = state_split),
    (state_split, input_split, time_horizon) ->
        linear_stochastically_switched_mixture(time_horizon; state_split = state_split),
    (40, 40),
    (1,),
    10,
)
...
function benchmark_intervalsyscore(name::String, direct = true)
    ...
    abstraction_time, mdp, spec, upper_bound_spec =
        measure_abstraction_time(problem, constructor)
    ...
    certification_time, V_lower, V_upper =
        measure_certification_time(prob, upper_bound_prob)
