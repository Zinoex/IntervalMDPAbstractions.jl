...
@testset "linear_transformation" begin
    ...
    region_1 = Zonotope([-0.625, -0.5], [0.125 0.5; 0.0 0.5])
    Y1_expected = concretize(
        ConvexHull(
        AffineMap(
            Tx.T * [1.0 0.1; -0.1 1.1] * Tx.Tinv,
            Intersection(region_1, X),
            Tx.T * [0.0, 0.5]
        ),
        AffineMap(
            Tx.T * [1.0 0.1; 0.0 1.1] * Tx.Tinv,
            Intersection(region_1, X),
            Tx.T * [0.0, 0.5]
        )
    ),
    )
    region_2 = Zonotope([-0.375, -0.5], [0.125 0.5; 0.0 0.5])
    Y2_expected = concretize(
        ConvexHull(
        AffineMap(
            Tx.T * [1.0 0.1; -0.2 1.1] * Tx.Tinv,
            Intersection(region_2, X),
            Tx.T * [0.0, 0.5]
        ),
        AffineMap(
            Tx.T * [1.0 0.1; 0.0 1.1] * Tx.Tinv,
