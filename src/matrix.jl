...
struct AtomicSparseMatrixCOO{Tv, Ti <: Integer} <: AbstractMatrix{Tv}
    ...
    function AtomicSparseMatrixCOO(m::Integer, n::Integer, rows::Vector{Ti}, cols::Vector{Ti}, values::Vector{Tv}) where {Tv, Ti}
...
AtomicSparseMatrixCOO{Tv, Ti}(::UndefInitializer, m::Integer, n::Integer) where {Tv, Ti} = AtomicSparseMatrixCOO(m, n, Ti[], Ti[], Tv[])
AtomicSparseMatrixCOO{Tv}(::UndefInitializer, m::Integer, n::Integer) where {Tv} = AtomicSparseMatrixCOO{Tv, Int32}(UndefInitializer(), m, n)
AtomicSparseMatrixCOO(::UndefInitializer, m::Integer, n::Integer) = AtomicSparseMatrixCOO{Float64}(UndefInitializer(), m, n)
...
function Base.setindex!(A::AtomicSparseMatrixCOO{Tv, Ti}, v, i::Integer, j::Integer) where {Tv, Ti}
